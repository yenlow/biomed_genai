...
test_table_name = f"{catalog}.{db}.pubmedqa"
...
pred_base = test \
    .withColumn("prediction", expr(f"""ai_query('{base_endpoint_name}',
                                   CONCAT('{{"messages": [{{"role": "user", "content": "', question, '. Answer with yes, no or maybe in lowercase.}}]}}'))"""))
display(pred_base)
...
# Eval base model
with mlflow.start_run(run_name=f"pubmedqa_{base_endpoint_name}") as run:
...
# Eval FT model
with mlflow.start_run(run_name=f"pubmedqa_{model_endpoint_name}") as run:
    results = mlflow.evaluate(
        data=pred_ft,
        targets="answer",
        predictions="prediction",
